...
Ryuka Landscape Designer v4.7.0 は、GitHub Pagesで配信できる静的なThree.jsアプリケーションです。
...
`three.min.js` → `GLTFLoader.js` → `fixed-site-data.js` → 既存material/model群 → `design-state.js` → `plant-editor.js` → `asset-catalog.js` → `asset-loader.js` → `app.js` の順です。`app.js`は起動直後に固定データを検証し、異常時はコンソールと画面上に警告します。
...
起動時は従来のプロシージャル施設を即時描画し、6つのGLBをバックグラウンドで読み込みます。完了通知は短いdebounceでまとめ、`facilities`、`guestBeds`、`herbs`だけを再構築します。失敗URLは自動で無限再試行せず、簡易モデルを維持します。
