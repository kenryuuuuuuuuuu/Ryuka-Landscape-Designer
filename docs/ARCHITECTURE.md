...
Ryuka Landscape Designer v4.2.0 は、GitHub Pagesで配信できる静的なThree.jsアプリケーションです。
...
`three.min.js` → `fixed-site-data.js` → `ground-materials.js` → `app.js` の順です。`app.js`は起動直後に固定データを検証し、異常時はコンソールと画面上に警告します。
