/* ============================================================
 Ryuka Landscape Designer v4.6.0
...
$('exportBtn').onclick=()=>{savePlan(true);const blob=new Blob([JSON.stringify({version:'4.6.0',quality:STATE.quality,plans:DESIGN.plans},null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='ryuka-landscape-plans.json';a.click();URL.revokeObjectURL(a.href)};$('importBtn').onclick=()=>$('importFile').click();$('importFile').onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{try{const q=JSON.parse(r.result);DESIGN.replacePlans(q.plans||q);const removed=DESIGN.cleanInvalid((plant,x,z)=>plantEditor.isValid(plant,x,z));plans=DESIGN.plans;if(q.quality&&['auto','high','low'].includes(q.quality)){STATE.quality=q.quality;localStorage.setItem('ryuka-render-quality',STATE.quality);applyQuality();updateSun()}readPlan('A',true);toast(removed?`設定を読み込み、不正な植栽${removed}件を除外しました`:'設定を読み込みました')}catch(error){console.error('[Ryuka] 設定読込エラー',error);toast('設定ファイルを読み込めません')}};r.readAsText(f)};
...
plantEditor=createPlantEditor({THREE,scene,renderer,data:DATA,designState:DESIGN,getCamera:()=>camera,getObjects:()=>plantObjects,rebuild:rebuildPlantLayout,toast,showInfo:showPlantEditorInfo,getViewCenter:()=>({x:cam.target.x,z:cam.target.z})});
window.RYUKA_DESIGN_API=Object.freeze({
 status:()=>({plan:STATE.activePlan,mode:STATE.mode,plants:resolvedPlants.map(p=>({id:p.designId,name:p.name,x:p.x,z:p.z,sourceType:p.sourceType})),selectable:selectable.length,geometries:renderer.info.memory.geometries,textures:renderer.info.memory.textures,lights:document.body.dataset.lightCount||'',editing:plantEditor.isEditing()}),
 begin:()=>plantEditor.begin(),end:()=>plantEditor.end(),add:name=>plantEditor.addSpecies(name),select:id=>plantEditor.select(id),move:(dx,dz)=>plantEditor.move(dx,dz),undo:()=>plantEditor.undo(),redo:()=>plantEditor.redo(),switchPlan:key=>readPlan(key),setMode,setSeason:season=>{if(['spring','summer','autumn','winter'].includes(season)){STATE.season=season;rebuildGrowth()}},setGrowthYear:year=>{STATE.growthYear=clamp(Number(year)||0,0,10);rebuildGrowth()}
});
$('plantEditToggle').onclick=()=>{if(plantEditor.isEditing())plantEditor.end();else{if(STATE.measure){STATE.measure=false;$('measureBtn').classList.remove('on');$('measureTip').style.display='none';clearMeasure()}if(cam.mode==='walk')stopWalk();plantEditor.begin()}};
