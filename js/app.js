...
objectEditor=createObjectEditor({THREE,scene,renderer,data:DATA,designState:DESIGN,getCamera:()=>camera,getObjects:()=>objectObjects,getPlants:()=>resolvedPlants,rebuild:rebuildObjectLayout,toast,showInfo:showObjectEditorInfo,beforeBegin:prepareObjectEditing,onBegin:buildLabels,onEnd:buildLabels,getViewCenter:()=>({x:cam.target.x,z:cam.target.z})});
...
$('importFile').onchange=event=>{const file=event.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{try{const payload=JSON.parse(reader.result);DESIGN.replacePlans(payload.plans||payload);const removedPlants=DESIGN.cleanInvalid((plant,x,z)=>plantEditor.isValid(plant,x,z));const removedObjects=DESIGN.cleanInvalidObjects((item,x,z,rotation)=>objectEditor.isValid(item,x,z,rotation));plans=DESIGN.plans;if(payload.quality&&['auto','high','low'].includes(payload.quality)){STATE.quality=payload.quality;localStorage.setItem('ryuka-render-quality',STATE.quality);applyQuality();updateSun()}if(payload.modelDetail&&['auto','detailed','simple'].includes(payload.modelDetail)){STATE.modelDetail=payload.modelDetail;localStorage.setItem('ryuka-model-detail',STATE.modelDetail);document.querySelectorAll('[data-model-detail]').forEach(button=>button.classList.toggle('on',button.dataset.modelDetail===STATE.modelDetail));syncAssetVariant();updateAssetStatus(ASSET_MANAGER.getStatus())}readPlan('A',true);const removed=removedPlants+removedObjects;toast(removed?`設定を読み込み、不正な配置${removed}件を除外しました`:'設定を読み込みました')}catch(error){console.error('[Ryuka] 設定読込エラー',error);toast('設定ファイルを読み込めません')}};reader.readAsText(file)};
...
function selectAt(x,y){if(plantEditor?.isEditing()||objectEditor?.isEditing())return;screenRay(x,y);let hits=raycaster.intersectObjects(selectable,true);let obj=hits.find(h=>{let o=h.object;while(o&&!o.userData.info)o=o.parent;h.infoObj=o;return!!o})?.infoObj;if(!obj)return;const info=obj.userData.info;$('insTitle').textContent=info.title;$('insBody').textContent=info.body;$('insMeta').innerHTML='';(info.meta||[]).forEach(a=>{$('insMeta').insertAdjacentHTML('beforeend',`<span>${a[0]}</span><span>${a[1]}</span>`)});$('inspector').classList.remove('hidden')}
