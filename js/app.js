/* ============================================================
 Ryuka Landscape Designer v4.9.0
...
const GROUND_FEATURE_RENDER_MATERIALS=createGroundFeatureMaterials(THREE,GROUND);
...
const BASE_GROUND_FEATURES=createBaseGroundFeatures(DATA);
...
let resolvedGroundFeatures=BASE_GROUND_FEATURES.map(item=>({...item,sourceType:'base'}));
const groundFeatureObjects=new Map();
let plantEditor=null,objectEditor=null,groundEditor=null;
...
function replaceGroundFeaturePreview(item,status){
 ...
 if(label)label.position.set(labelPoint.x,label.position.y,labelPoint.z);
 if(!entry)return;
 if(previous){const members=new Set();previous.group.traverse(object=>members.add(object));for(let index=selectable.length-1;index>=0;index--)if(members.has(selectable[index]))selectable.splice(index,1);previous.group.traverse(object=>{object.geometry?.dispose?.();if(object.material&&!sharedMaterials.has(object.material))object.material.dispose?.()});groups.groundFeatures.remove(previous.group)}
 tag(entry.group,groundInfo(item));entry.group.visible=STATE.layers[item.layer]!==false;groups.groundFeatures.add(entry.group);groundFeatureObjects.set(item.designId,{...entry,label})
}
...
function buildHerbs(){clearRebuildGroup(groups.herbs);const H=DATA.herbs;H.clusters.forEach((p,i)=>{for(let k=0;k<7;k++){const a=k/7*Math.PI*2,r=.2+(.08*(k%2));addPlantCluster(groups.herbs,p.x+Math.sin(a)*r,p.z+Math.cos(a)*r,0x776f9c,.11,300+i*20+k)}});addPlantCluster(groups.herbs,H.accent.x,H.accent.z,0x4f7648,.38,350)}
...
const DESIGN=createDesignState({baseTrees:DATA.trees,baseObjects:BASE_OBJECTS,baseGroundFeatures:BASE_GROUND_FEATURES,defaults:PLAN_DEFAULTS,storageKey:'ryuka-v4-plans'});
...
function buildRotations(){clearRebuildGroup(groups.rotations);const density=densityFactor(),ridgeMaterial=STATE.mode==='real'?GROUND.ridgeSoil:GROUND.planSoil;DATA.rotations.forEach((fixed,bi)=>{const feature=resolvedGroundFeatures.find(item=>item.designId===`base-ground-rotation-${bi}`),points=feature?.points||[{x:fixed.cx-fixed.w/2,z:fixed.cz-fixed.d/2},{x:fixed.cx+fixed.w/2,z:fixed.cz-fixed.d/2},{x:fixed.cx+fixed.w/2,z:fixed.cz+fixed.d/2},{x:fixed.cx-fixed.w/2,z:fixed.cz+fixed.d/2}];if(!GROUND_GEOMETRY_UTILS.isSimplePolygon(points))return;const xs=points.map(point=>point.x),minX=Math.min(...xs),maxX=Math.max(...xs),w=maxX-minX,n=Math.max(1,Math.round(w/1.5));for(let i=0;i<n;i++){const x=minX+.75+i*Math.max(0,w-1.5)/Math.max(1,n-1),intervals=GROUND_GEOMETRY_UTILS.verticalPolygonIntervals(points,x);intervals.forEach((interval,intervalIndex)=>{const d=interval.maxZ-interval.minZ;if(d<=.65)return;const cz=(interval.minZ+interval.maxZ)/2;groups.rotations.add(soilRidge(.75,d,ridgeMaterial,x,cz));const rows=Math.max(2,Math.round(Math.max(.8,d-.8)/(.75/density)));for(let j=0;j<rows;j++){const z=interval.minZ+.4+j*Math.max(0,d-.8)/Math.max(1,rows-1);if(!GROUND_GEOMETRY_UTILS.pointInPolygon({x,z},points))continue;if(STATE.season!=='winter'||bi===3)addPlantCluster(groups.rotations,x,z,SEASON_COLORS[STATE.season][bi%2],.12+.055*density,740+bi*200+i*30+intervalIndex*17+j)}})}})}
...
function rebuildGroundLayout(){buildGroundFeatures();buildRotations();buildLabels();updateGroundTotals();updateResourceMetrics()}
function rebuildGrowth(){buildGroundFeatures();buildTrees();buildRotations();buildGuestBeds();buildHerbs();buildLawn();buildObjects();buildLabels();applyGrowthUI();updateGroundTotals();updateResourceMetrics()}
...
function readPlan(k,skipSave=false){if(!skipSave)savePlan(true);plantEditor?.deselect();objectEditor?.deselect();groundEditor?.deselect();STATE.activePlan=k;DESIGN.setActivePlan(k);plans=DESIGN.plans;Object.assign(STATE,plans[k]);rebuildGrowth()}
...
function preparePlantEditing(){
 ...
 if(groundEditor?.isEditing())groundEditor.end();
...
plantEditor=createPlantEditor({THREE,scene,renderer,data:DATA,designState:DESIGN,getCamera:()=>camera,getObjects:()=>plantObjects,getGroundFeatures:()=>resolvedGroundFeatures,rebuild:rebuildPlantLayout,toast,showInfo:showPlantEditorInfo,beforeBegin:preparePlantEditing,getViewCenter:()=>({x:cam.target.x,z:cam.target.z})});
...
objectEditor=createObjectEditor({THREE,scene,renderer,data:DATA,designState:DESIGN,getCamera:()=>camera,getObjects:()=>objectObjects,getPlants:()=>resolvedPlants,getGroundFeatures:()=>resolvedGroundFeatures,rebuild:rebuildObjectLayout,toast,showInfo:showObjectEditorInfo,beforeBegin:prepareObjectEditing,onBegin:buildLabels,onEnd:buildLabels,onVisibilityChange:buildLabels,getViewCenter:()=>({x:cam.target.x,z:cam.target.z})});
function showGroundEditorInfo(item,status,vertex){$('insTitle').textContent=item.label;$('insBody').textContent=`${item.sourceType==='base'?'固定地表':'追加地表'}・プラン${STATE.activePlan}・${status.state}`;$('insMeta').innerHTML='';const rows=[['種類',item.kind==='path'?'園路':'区画'],['素材',GROUND_FEATURE_MATERIALS[item.materialId]?.label||item.materialId],['レイヤー',item.layer],['頂点',`${item.points.length}${vertex>=0?`（選択 ${vertex+1}）`:''}`],['配置状態',status.state]];if(item.kind==='path')rows.push(['中心線延長',`${item.length.toFixed(2)}m`],['幅',`${item.width.toFixed(2)}m`],['面積',`${item.area.toFixed(2)}㎡`]);else rows.push(['面積',`${item.area.toFixed(2)}㎡`],['周長',`${item.perimeter.toFixed(2)}m`]);if(item.sourceType==='base')rows.push(['元面積',`${GROUND_GEOMETRY_UTILS.polygonArea(item.kind==='path'?GROUND_GEOMETRY_UTILS.buildPathRibbon(item.basePoints,item.baseWidth):item.basePoints).toFixed(2)}㎡`],['面積差',`${(item.area-GROUND_GEOMETRY_UTILS.polygonArea(item.kind==='path'?GROUND_GEOMETRY_UTILS.buildPathRibbon(item.basePoints,item.baseWidth):item.basePoints)).toFixed(2)}㎡`]);rows.forEach(row=>$('insMeta').insertAdjacentHTML('beforeend',`<span>${row[0]}</span><span>${row[1]}</span>`));$('inspector').classList.remove('hidden')}
function updateGroundSelectionUI(item){const info=$('groundSelectedInfo'),material=$('groundMaterial'),widthRow=$('groundWidthRow');if(!item){info.innerHTML='<span>園路・区画を選択してください</span>';material.innerHTML='';widthRow.style.display='none';return}info.innerHTML=`<strong>${item.label}</strong><span>${item.sourceType==='base'?'固定':'追加'}・${item.points.length}頂点</span><span>${item.kind==='path'?`延長 ${item.length.toFixed(2)}m・面積 ${item.area.toFixed(2)}㎡`:`面積 ${item.area.toFixed(2)}㎡・周長 ${item.perimeter.toFixed(2)}m`}</span>`;material.innerHTML='';Object.entries(GROUND_FEATURE_MATERIALS).filter(([,value])=>value.kind===item.kind).forEach(([id,value])=>{const option=document.createElement('option');option.value=id;option.textContent=value.label;option.selected=id===item.materialId;material.appendChild(option)});widthRow.style.display=item.kind==='path'?'grid':'none';$('groundWidth').value=item.width||1.2;$('groundWidthOut').textContent=`${(item.width||0).toFixed(1)}m`;document.querySelectorAll('[data-ground-action="width-down"],[data-ground-action="width-up"]').forEach(button=>button.disabled=item.kind!=='path')}
function updateGroundTotals(){if(!DESIGN?.resolveGroundFeatures)return;const totals=groundFeatureTotals(DESIGN.resolveGroundFeatures(STATE.activePlan)),areas=totals.areaByMaterial,green=(areas['area-lawn']||0)+(areas['area-clover']||0)+(areas['area-crimson-clover']||0)+(areas['area-green-manure']||0),gravel=(areas['area-gravel']||0)+(areas['area-yard-gravel']||0),garden=(areas['area-vegetable']||0)+(areas['area-flower-bed']||0)+(areas['area-herb']||0);$('groundTotals').innerHTML=`<strong>地表集計</strong><dl><dt>園路延長</dt><dd>${totals.pathLength.toFixed(1)}m</dd><dt>園路面積</dt><dd>${totals.pathArea.toFixed(1)}㎡</dd><dt>芝・クローバー・緑肥</dt><dd>${green.toFixed(1)}㎡</dd><dt>砂利</dt><dd>${gravel.toFixed(1)}㎡</dd><dt>菜園・花壇</dt><dd>${garden.toFixed(1)}㎡</dd><dt>防草</dt><dd>${(areas['area-weed-control']||0).toFixed(1)}㎡</dd></dl>`}
groundEditor=createGroundFeatureEditor({THREE,scene,renderer,data:DATA,designState:DESIGN,getCamera:()=>camera,getObjects:()=>groundFeatureObjects,getFeatures:()=>resolvedGroundFeatures,getResolvedObjects:()=>resolvedObjects,getResolvedPlants:()=>resolvedPlants,rebuild:rebuildGroundLayout,preview:replaceGroundFeaturePreview,toast,showInfo:showGroundEditorInfo,beforeBegin:prepareGroundEditing,onBegin:()=>{buildGroundFeatures();buildLabels()},onEnd:()=>{buildGroundFeatures();buildLabels()},onVisibilityChange:buildLabels,onSelectionChange:updateGroundSelectionUI,updateTotals:updateGroundTotals,getViewCenter:()=>({x:cam.target.x,z:cam.target.z})});
...
window.RYUKA_GROUND_API=Object.freeze({
 status:()=>({plan:STATE.activePlan,mode:STATE.mode,editing:groundEditor.isEditing(),selectedId:groundEditor.selectedId,selectedVertex:groundEditor.selectedVertex,selectedSegment:groundEditor.selectedSegment,features:resolvedGroundFeatures.length,totals:groundEditor.totals(),outlinesVisible:Array.from(groundFeatureObjects.values()).filter(entry=>entry.outline?.visible).length,selectable:selectable.length,geometries:renderer.info.memory.geometries,textures:renderer.info.memory.textures}),
 begin:()=>groundEditor.begin(),end:()=>groundEditor.end(),add:type=>groundEditor.add(type),select:id=>groundEditor.select(id),selectVertex:index=>groundEditor.selectVertex(Number(index)),move:(dx,dz)=>groundEditor.move(Number(dx),Number(dz)),moveAll:(dx,dz)=>groundEditor.moveAll(Number(dx),Number(dz)),addVertex:index=>groundEditor.addVertex(index),removeVertex:()=>groundEditor.removeVertex(),setWidth:value=>groundEditor.setWidth(value),setMaterial:id=>groundEditor.setMaterial(id),duplicate:()=>groundEditor.duplicate(),remove:()=>groundEditor.remove(),resetSelected:()=>groundEditor.resetSelected(),resetPlan:()=>groundEditor.resetPlan(),undo:()=>groundEditor.undo(),redo:()=>groundEditor.redo(),list:()=>groundEditor.list().map(item=>{const entry=groundFeatureObjects.get(item.designId);return{...item,visible:entry?.group.visible!==false,labelVisible:groups.labels.visible&&entry?.label?.parent===groups.labels,labelPosition:entry?.label?{x:entry.label.position.x,z:entry.label.position.z}:null}}),totals:()=>groundEditor.totals()
});
...
$('groundEditToggle').onclick=()=>{if(groundEditor.isEditing())groundEditor.end();else groundEditor.begin()};
$('groundUndoBtn').onclick=()=>groundEditor.undo();$('groundRedoBtn').onclick=()=>groundEditor.redo();$('groundSnap').value=localStorage.getItem('ryuka-ground-snap')||'0.25';$('groundSnap').onchange=event=>groundEditor.setSnap(event.target.value);$('groundAddVertexBtn').onclick=()=>groundEditor.addVertex();$('groundRemoveVertexBtn').onclick=()=>groundEditor.removeVertex();$('groundResetSelectedBtn').onclick=()=>groundEditor.resetSelected();$('groundDuplicateBtn').onclick=()=>groundEditor.duplicate();$('groundDeleteBtn').onclick=()=>groundEditor.remove();$('groundPlanResetBtn').onclick=()=>groundEditor.resetPlan();$('groundMaterial').onchange=event=>groundEditor.setMaterial(event.target.value);const groundWidthInput=$('groundWidth');groundWidthInput.addEventListener('pointerdown',()=>groundEditor.beginWidthPreview());groundWidthInput.addEventListener('focus',()=>groundEditor.beginWidthPreview());groundWidthInput.oninput=event=>{groundEditor.previewWidth(event.target.value);$('groundWidthOut').textContent=`${Number(event.target.value).toFixed(1)}m`};groundWidthInput.onchange=()=>groundEditor.finishWidthPreview(false);groundWidthInput.addEventListener('blur',()=>groundEditor.finishWidthPreview(false));groundWidthInput.addEventListener('keydown',event=>{if(event.key==='Escape'){groundEditor.finishWidthPreview(true);event.preventDefault()}});
GROUND_FEATURE_CATALOG.forEach(profile=>{const card=document.createElement('div');card.className='plant-card';card.innerHTML=`<strong>${profile.label}</strong><span>${profile.kind==='path'?`幅 ${profile.defaultWidth.toFixed(1)}m`:'面区画'}<br>${profile.description}</span><button class="action" type="button">追加</button>`;card.querySelector('button').onclick=()=>groundEditor.add(profile.featureType);$('groundCatalog').appendChild(card)});
document.querySelectorAll('[data-ground-action]').forEach(button=>button.onclick=()=>{const action=button.dataset.groundAction;if(action==='left')groundEditor.move(-1,0);if(action==='right')groundEditor.move(1,0);if(action==='up')groundEditor.move(0,-1);if(action==='down')groundEditor.move(0,1);if(action==='prev'){const item=resolvedGroundFeatures.find(feature=>feature.designId===groundEditor.selectedId);if(item)groundEditor.selectVertex((groundEditor.selectedVertex-1+item.points.length)%item.points.length)}if(action==='next'){const item=resolvedGroundFeatures.find(feature=>feature.designId===groundEditor.selectedId);if(item)groundEditor.selectVertex((groundEditor.selectedVertex+1)%item.points.length)}if(action==='add-vertex')groundEditor.addVertex();if(action==='remove-vertex')groundEditor.removeVertex();if(action==='move-mode')groundEditor.selectVertex(-1);if(action==='width-down'){const item=resolvedGroundFeatures.find(feature=>feature.designId===groundEditor.selectedId);if(item)groundEditor.setWidth(item.width-.1)}if(action==='width-up'){const item=resolvedGroundFeatures.find(feature=>feature.designId===groundEditor.selectedId);if(item)groundEditor.setWidth(item.width+.1)}if(action==='reset')groundEditor.resetSelected();if(action==='duplicate')groundEditor.duplicate();if(action==='delete')groundEditor.remove()});
$('exportBtn').onclick=()=>{savePlan(true);const blob=new Blob([JSON.stringify({version:'4.9.0',quality:STATE.quality,modelDetail:STATE.modelDetail,plans:DESIGN.plans},null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='ryuka-landscape-plans.json';a.click();URL.revokeObjectURL(a.href)};
$('importFile').onchange=event=>{const file=event.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{try{const payload=JSON.parse(reader.result);DESIGN.replacePlans(payload.plans||payload);const removedPlants=DESIGN.cleanInvalid((plant,x,z)=>plantEditor.isValid(plant,x,z));const removedObjects=DESIGN.cleanInvalidObjects((item,x,z,rotation,planKey,context)=>objectEditor.isValid(item,x,z,rotation,context));const removedGround=DESIGN.cleanInvalidGroundFeatures((item,planKey,context)=>groundEditor.isValid(item,context));plans=DESIGN.plans;if(payload.quality&&['auto','high','low'].includes(payload.quality)){STATE.quality=payload.quality;localStorage.setItem('ryuka-render-quality',STATE.quality);applyQuality();updateSun()}if(payload.modelDetail&&['auto','detailed','simple'].includes(payload.modelDetail)){STATE.modelDetail=payload.modelDetail;localStorage.setItem('ryuka-model-detail',STATE.modelDetail);document.querySelectorAll('[data-model-detail]').forEach(button=>button.classList.toggle('on',button.dataset.modelDetail===STATE.modelDetail));syncAssetVariant();updateAssetStatus(ASSET_MANAGER.getStatus())}readPlan('A',true);const removed=removedPlants+removedObjects+removedGround;toast(removed?`設定を読み込み、不正な配置${removed}件を除外しました`:'設定を読み込みました')}catch(error){console.error('[Ryuka] 設定読込エラー',error);toast('設定ファイルを読み込めません')}};reader.readAsText(file)};
...
$('clearMeasureBtn').onclick=clearMeasure;$('measureBtn').onclick=()=>{if(plantEditor?.isEditing()||objectEditor?.isEditing()||groundEditor?.isEditing()){toast('編集モードを終了してから計測してください');return}STATE.measure=!STATE.measure;$('measureBtn').classList.toggle('on',STATE.measure);$('measureTip').style.display=STATE.measure?'block':'none';clearMeasure()};
...
function selectAt(x,y){if(plantEditor?.isEditing()||objectEditor?.isEditing()||groundEditor?.isEditing())return;screenRay(x,y);let hits=raycaster.intersectObjects(selectable,true);let obj=hits.find(h=>{if(!isEffectivelyVisible(h.object))return false;let o=h.object;while(o&&!o.userData.info)o=o.parent;h.infoObj=o;return!!o})?.infoObj;if(!obj)return;const info=obj.userData.info;$('insTitle').textContent=info.title;$('insBody').textContent=info.body;$('insMeta').innerHTML='';(info.meta||[]).forEach(a=>{$('insMeta').insertAdjacentHTML('beforeend',`<span>${a[0]}</span><span>${a[1]}</span>`)});$('inspector').classList.remove('hidden')}
