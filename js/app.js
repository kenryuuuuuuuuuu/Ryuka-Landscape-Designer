...
function buildRotations(){clearRebuildGroup(groups.rotations);const density=densityFactor(),names=CROP_NAMES[STATE.cropPattern],ridgeMaterial=STATE.mode==='real'?GROUND.ridgeSoil:GROUND.planSoil;DATA.rotations.forEach((fixed,bi)=>{const feature=resolvedGroundFeatures.find(item=>item.designId===`base-ground-rotation-${bi}`),points=feature?.points||[{x:fixed.cx-fixed.w/2,z:fixed.cz-fixed.d/2},{x:fixed.cx+fixed.w/2,z:fixed.cz-fixed.d/2},{x:fixed.cx+fixed.w/2,z:fixed.cz+fixed.d/2},{x:fixed.cx-fixed.w/2,z:fixed.cz+fixed.d/2}],xs=points.map(point=>point.x),zs=points.map(point=>point.z),minX=Math.min(...xs),maxX=Math.max(...xs),minZ=Math.min(...zs),maxZ=Math.max(...zs),w=maxX-minX,d=maxZ-minZ,cz=(minZ+maxZ)/2,n=Math.max(1,Math.round(w/1.5));for(let i=0;i<n;i++){const x=minX+.75+i*Math.max(0,w-1.5)/Math.max(1,n-1);if(!GROUND_GEOMETRY_UTILS.pointInPolygon({x,z:cz},points))continue;groups.rotations.add(soilRidge(.75,d,ridgeMaterial,x,cz));const rows=Math.max(2,Math.round(Math.max(.8,d-.8)/(.75/density)));for(let j=0;j<rows;j++){const z=minZ+.4+j*Math.max(0,d-.8)/Math.max(1,rows-1);if(!GROUND_GEOMETRY_UTILS.pointInPolygon({x,z},points))continue;if(STATE.season!=='winter'||bi===3)addPlantCluster(groups.rotations,x,z,SEASON_COLORS[STATE.season][bi%2],.12+.055*density,740+bi*200+i*30+j)}}})}
