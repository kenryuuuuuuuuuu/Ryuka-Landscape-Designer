...
function buildSite(){siteMeshes.forEach(o=>{groups.site.remove(o);o.geometry?.dispose()});siteMeshes=[];const n=clipPoly(DATA.site,splitT,true),s=clipPoly(DATA.site,splitT,false);const mn=meshShape(n,STATE.mode==='real'?GROUND.takuchi:GROUND.planTak,.002),ms=meshShape(s,STATE.mode==='real'?GROUND.field:GROUND.planField,.003);groups.site.add(mn,ms);siteMeshes.push(mn,ms);tag(mn,{title:'北側宅地',body:'建物・進入路・作業ヤードを含む宅地想定エリア。',meta:[['面積',northAreaAt(splitT).toFixed(1)+'㎡'],['基準','登記値319㎡']]});tag(ms,{title:'南側の畑',body:'収穫体験、輪作、果樹、ハーブ、広場、パーゴラをまとめたランドスケープエリア。',meta:[['面積',(polyArea(DATA.site)-northAreaAt(splitT)).toFixed(1)+'㎡']]});
...
// ---------- paths / facilities ----------
function buildPaths(){groups.paths.clear();const material=STATE.mode==='real'?GROUND.path:GROUND.planPath;DATA.paths.forEach((p,i)=>{const m=meshShape(p,material,.045);groups.paths.add(tag(m,{title:'園路・作業動線',body:'建物、井戸、輪作区画、広場をつなぐ通路。元HTMLの四つのポリゴン座標を保持しています。',meta:[['区間',String(i+1)]]}))})}
function buildFacilities(){groups.facilities.clear();const F=DATA.facilities,yard=meshShape(F.yard,STATE.mode==='real'?GROUND.yardGravel:GROUND.planGravel,.05);groups.facilities.add(tag(yard,{title:'作業ヤード',body:'道具収納、資材仮置き、畑作業の起点となる砕石エリア。',meta:[['位置','東側・建物南東']] }));
...
function buildRotations(){
  ...
  DATA.rotations.forEach((b,bi)=>{
    const patch=meshShape([{x:b.cx-b.w/2,z:b.cz-b.d/2},{x:b.cx+b.w/2,z:b.cz-b.d/2},{x:b.cx+b.w/2,z:b.cz+b.d/2},{x:b.cx-b.w/2,z:b.cz+b.d/2}],STATE.mode==='real'?GROUND.rotationSoil:GROUND.planSoil,.037);
    ...
    for(let i=0;i<n;i++){
      ...
      groups.rotations.add(soilRidge(.75,b.d,STATE.mode==='real'?GROUND.ridgeSoil:GROUND.planSoil,x,b.cz));
...
function buildLawn(){groups.lawn.clear();const L=DATA.lawn,lawnMaterial=STATE.mode==='real'?GROUND.clover:GROUND.planClover,a=meshShape(L.west,lawnMaterial,.04),b=meshShape(L.east,lawnMaterial,.04);groups.lawn.add(tag(a,{title:'クローバー広場',body:'畑の中に余白と滞在場所をつくる低草地。パーゴラへの動線と果樹景観をつなぎます。',meta:[['位置','南側中央〜西']]}),b);
