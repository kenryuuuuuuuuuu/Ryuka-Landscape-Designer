...
assert(workspaces.exterior.sections.includes('walkthrough'));
...
const environmentSource = source('js/environment-model.js');
assert(!environmentSource.includes('GEOMETRIES.road'), 'legacy shared environment road must be removed');
...
assert(materialSource.includes('color: 0x5e442e'), 'dark cedar siding must be the material default');
const appearanceIndexSource = source('index.html');
assert(appearanceIndexSource.includes('name="sugiColor" value="#5e442e" checked'), 'dark cedar siding must be selected by default');
assert(appearanceIndexSource.includes('name="wallColor" value="#26384a">ネイビー'), 'navy wall color option is required');
...
assert(appSource.includes("fetch(location.href,{method:'HEAD',cache:'no-store'})"), 'version date must use the deployed document Last-Modified header');
assert(appSource.includes("south:{target:[1,2,0],r:62,a:0,p:1.18}"), 'south camera must be positioned south of the site');
